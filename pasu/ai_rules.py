...
def calculate_risk(animal, symptom, additional_symptoms, duration, num_mortality=0, image_path=None, notes=None):
    ...
    if gemini_api_key and image_path and os.path.exists(image_path):
        try:
            ...
            # If we get a recommendation from the AI, we could use it, but the existing codebase uses get_recommendation separately.
            # We will append the disease to the reasons list so the UI shows it.
...
def get_recommendation(risk_level, symptom=None, additional_symptoms=None, notes=None, image_description=None):
    try:
        ...
        if response and "Sorry, I am facing an issue" not in response:
            return response
    except Exception as e:
    ...
    if risk_level == "HIGH":
        return (
            "URGENT: Contact a qualified veterinarian immediately. "
            "Keep the affected animal separated from other animals "
            "until professional advice is obtained. "
            "Provide clean drinking water and avoid giving medicines "
            "without veterinary advice."
        )
    elif risk_level == "MEDIUM":
        return (
            "Monitor the animal closely. Provide clean water, "
            "adequate rest and appropriate nutrition. "
            "Contact a veterinarian if symptoms persist, worsen, "
            "or new symptoms appear."
        )
    else:
        return (
            "Continue observation. Provide clean water, proper "
            "nutrition and a clean environment. "
