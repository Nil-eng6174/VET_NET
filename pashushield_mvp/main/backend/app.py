...
def detect_clusters(all_reports):
    valid_points = []
    for r in all_reports:
        ...
        if not lat or not lng:
            if loc in FALLBACK_COORDS:
                lat, lng = FALLBACK_COORDS[loc]
            else:
                continue
        try:
            valid_points.append({
                'lat': float(lat), 'lng': float(lng),
                'symptom': r.get('symptom'), 'animal': r.get('animal'),
                'locality': loc
            })
        except:
            continue
    ...
    for i, p1 in enumerate(valid_points):
        ...
        for j, p2 in enumerate(valid_points):
            ...
            if p1['symptom'] == p2['symptom']:
                dist = haversine(p1['lat'], p1['lng'], p2['lat'], p2['lng'])
                if dist <= 10.0:
                    neighbors.append(j)
        
        if len(neighbors) >= 3:
            for n in neighbors: visited.add(n)
            c_lat = sum(valid_points[n]['lat'] for n in neighbors) / len(neighbors)
            c_lng = sum(valid_points[n]['lng'] for n in neighbors) / len(neighbors)
            clusters.append({
                'center': [c_lat, c_lng], 'radius_km': 10.0,
                'symptom': p1['symptom'], 'animal': p1['animal'], 'count': len(neighbors),
                'message': f"🚨 EMERGING CLUSTER: {len(neighbors)} cases of {p1['symptom']} detected within 10km radius in/near {p1['locality']}."
            })
    return clusters
...
def vet_dashboard_data():
    ...
    clusters = detect_clusters(recent_reports)
    for c in clusters:
        alerts.insert(0, c['message'])
